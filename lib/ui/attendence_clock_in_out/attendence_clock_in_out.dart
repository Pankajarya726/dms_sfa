...
import 'package:sfa/ui/attendence_clock_in_out/clock%20in%20bloc/clock_in_bloc.dart';
import 'package:sfa/ui/attendence_clock_in_out/clock%20in%20bloc/clock_in_states.dart';
import 'package:sfa/ui/attendence_clock_in_out/clock%20out%20bloc/clock_out_bloc.dart';
...
class _AttendenceClockInOutState extends State<AttendenceClockInOut> {
  ...
  XFile? _image;
  ClockInBloc clockInBloc = ClockInBloc();
  ClockOutBloc clockOutBloc = ClockOutBloc();
    ...
    return SingleChildScrollView(
      ...
      child: MultiBlocProvider(
        providers: [
          ...
          BlocProvider(
            create: (context) => clockOutBloc,
          )
        ],
        child: clockInOut == false ? clockOutLayout() : clockInLayout(),
      ),
    );
  }
  ...
  Widget clockOutLayout() {
    setClockInOutValues();
    return BlocBuilder<ClockOutBloc, ClockOutStates>(
      builder: (context, state) {
        if (state is ClockOutInitialState) {
          clockOutBloc.add(ClockOutSuccessEvent());
        }
  ...
  Widget clockInLayout() {
    return BlocBuilder<ClockInBloc, ClockInStates>(
      builder: (context, state) {
        if (state is ClockInInitialState) {
          clockInBloc.add(ClockInSuccessEvent());
        }
  ...
  void setTimerValues() async {}

  void getTimerValues() async {}

  void setClockInOutValues() async {
    sharedPreferences!.setBool("clock_in_out", clockInOut);
    print(clockInOut);
  }

  void getClockInOutValues() async {
    clockInOut = sharedPreferences!.getBool("clock_in_out")!;
  }
}
