...
class _StartDayScreenState extends State<StartDayScreen> {
                          ...
                          children: [
                            ...
                            BlocBuilder<AddPlanBloc, AddPlanStates>(
                              builder: (context, state) {
                                ...
                                return selectedPrimaryTag!.secondaryTagType == "drop_down"
                                    ? Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                          const SizedBox(
                                          ...
                                          TextFormField(
                                            ...
                                            onTap: () async {
                                              ...
                                              RenderObject? object = globalKey2.currentContext!.findRenderObject();
                                              object!.showOnScreen();
                                            },
                                          ),
                                        ],
  ...
  void submit() async {
    ...
    if (selectedPrimaryTag!.name.trim().toLowerCase() == "leave" || selectedPrimaryTag!.name.trim().toLowerCase() == "holiday") {
      ...
      DateTime _ntpTime = await NTP.now().timeout(const Duration(seconds: 3), onTimeout: () {
        return DateTime.now();
      });
    ...
    //otherwise else will execute
      ...
      if (latitude == 0.0 && longitude == 0.0) {
        Utility.showToast("Unable to fetch your location, Please try again.");
        return;
      ...
      input["start_day_address"] =
          currentAddress == "Click here to get current location!" || currentAddress == "" ? "Location not found" : currentAddress;
      input["start_day_time"] = "${_ntpTime.hour}:${_ntpTime.minute}:${_ntpTime.second}";
