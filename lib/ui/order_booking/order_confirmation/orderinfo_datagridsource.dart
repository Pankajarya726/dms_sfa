...
class OrderInfoDataGridSource extends DataGridSource {
  ...
  OrderInfoDataGridSource({required this.isWebOrDesktop, required this.orderDataCount}) {
    ...
    if (isWebOrDesktop) {
      return DataGridRowAdapter(color: backgroundColor, cells: <Widget>[
        ...
        Container(
          padding: const EdgeInsets.all(8),
          alignment: Alignment.centerRight,
          child: Text(NumberFormat.currency(locale: 'en_US', symbol: r'$').format(row.getCells()[3].value)),
        ),
        Container(
          padding: const EdgeInsets.all(8),
          alignment: Alignment.centerLeft,
        ...
        Container(
          padding: const EdgeInsets.all(8),
          alignment: Alignment.centerRight,
          child: Text(NumberFormat.currency(locale: 'en_US', symbol: r'$', decimalDigits: 0).format(row.getCells()[5].value)),
        ),
      ]);
    } else {
      Widget buildWidget({
      ...
      return DataGridRowAdapter(
          ...
          cells: row.getCells().map<Widget>((DataGridCell dataCell) {
            if (dataCell.columnName == 'id' || dataCell.columnName == 'customerId') {
  ...
  Widget? buildTableSummaryCellWidget(
    ...
    Widget buildCell(String value, EdgeInsets padding, Alignment alignment) {
      return Container(
        padding: padding,
        alignment: alignment,
        child: Text(value, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w500)),
      );
    }
...
class OrderInfo {
  ...
  OrderInfo(this.id, this.customerId, this.name, this.freight, this.city, this.price);
